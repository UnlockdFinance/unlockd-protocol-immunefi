...
export const setPricesInNFTOracle = async (
  ...
  for (const [assetSymbol, assetAddress] of Object.entries(assetsAddresses) as [
    ...
    const priceIndex = Object.keys(prices).findIndex(
      (value) => value === assetSymbol
    );
    ...
    const [, price] = (Object.entries(prices) as [string, string][])[
      priceIndex
    ];
    console.log("setPricesInNFTOracle", assetSymbol, assetAddress, price);
    const maxSupplyIndex = Object.keys(assetsMaxSupply).findIndex(
      (value) => value === assetSymbol
    );
    if (maxSupplyIndex == undefined) {
      console.log(
        "can not find max supply for asset",
        assetSymbol,
        assetAddress
      );
      continue;
    ...
    const [, maxSupply] = (
      Object.entries(assetsMaxSupply) as [string, string][]
    )[maxSupplyIndex];
    const tokenIds = [...Array(parseInt(maxSupply)).keys()];

    for (const tokenId of tokenIds) {
      await waitForTx(
        await nftOracleInstance.setNFTPrice(assetAddress, tokenId, price)
      );
    }
  }
};
    ...
    tokenIds.map(
      async (tokenId) =>
        await waitForTx(
          await nftOracleInstance
            .connect(oracleOwnerSigner)
            .setNFTPrice(assetAddress, tokenId, price)
        )
    );
  }
};
...
export const deployAllChainlinkMockAggregators = async (
  ...
  for (const tokenContractName of Object.keys(initialPrices)) {
    if (tokenContractName !== "ETH") {
      ...
      aggregators[tokenContractName] = await deployChainlinkMockAggregator(
        tokenContractName,
        decimals,
        price,
        verify
...
export const deployAllReservesMockAggregatorsInPoolConfig = async (
