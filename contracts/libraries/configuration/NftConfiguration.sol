...
library NftConfiguration {
     ...
     **/
    function setConfigTimestamp(
        DataTypes.NftConfigurationMap memory self,
        uint256 configTimestamp
    ) internal pure {
        require(
            configTimestamp <= MAX_VALID_CONFIG_TIMESTAMP,
            Errors.RC_INVALID_MAX_CONFIG_TIMESTAMP
        );

        self.data =
            (self.data & CONFIG_TIMESTAMP_MASK) |
            (configTimestamp << CONFIG_TIMESTAMP_START_BIT_POSITION);
    }
     ...
     **/
    function getConfigTimestamp(
        DataTypes.NftConfigurationMap storage self
    ) internal view returns (uint256) {
        return ((self.data & ~CONFIG_TIMESTAMP_MASK) >>
            CONFIG_TIMESTAMP_START_BIT_POSITION);
    }
     ...
     **/
    function getConfigTimestampMemory(
        DataTypes.NftConfigurationMap memory self
    ) internal pure returns (uint256) {
        return ((self.data & ~CONFIG_TIMESTAMP_MASK) >>
            CONFIG_TIMESTAMP_START_BIT_POSITION);
