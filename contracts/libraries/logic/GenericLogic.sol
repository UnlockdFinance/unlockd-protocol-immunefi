...
library GenericLogic {
    ...
    function calculateLoanData(
        ...
        (vars.nftLtv, vars.nftLiquidationThreshold, ) = nftConfig
            .getCollateralParams();
    ...
    function calculateNftCollateralData(
        ...
        // calculate total collateral balance for the nft
        // all asset price has converted to ETH based, unit is in WEI (18 decimals)
        vars.nftUnitPrice = INFTOracleGetter(nftOracle).getNFTPrice(
            nftAddress,
            nftTokenId
        );
    ...
    function calculateLoanLiquidatePrice(
        ...
        (vars.ltv, vars.liquidationThreshold, vars.liquidationBonus) = nftConfig
            .getCollateralParams();

        vars.nftPriceInETH = INFTOracleGetter(nftOracle).getNFTPrice(
            nftAsset,
            nftTokenId
        );
