...
library ValidationLogic {
    ...
    function validateBorrow(
        ...
        require(
            reserveData.uTokenAddress != address(0),
            Errors.VL_INVALID_RESERVE_ADDRESS
        );
        require(
            nftData.uNftAddress != address(0),
            Errors.LPC_INVALID_UNFT_ADDRESS
        );
        ...
        if (loanId != 0) {
            ...
            require(
                params.asset == loanData.reserveAsset,
                Errors.VL_SPECIFIED_RESERVE_NOT_BORROWED_BY_USER
            );
            require(
                params.onBehalfOf == loanData.borrower,
        ...
        require(
            (nftConfig.getConfigTimestamp() +
                ILendPool(address(this)).getTimeframe()) >= block.timestamp,
            Errors.VL_TIMEFRAME_EXCEEDED
        );

        (vars.currentLtv, vars.currentLiquidationThreshold, ) = nftConfig
            .getCollateralParams();
        ...
        vars.amountOfCollateralNeeded = (vars.userBorrowBalance + params.amount)
