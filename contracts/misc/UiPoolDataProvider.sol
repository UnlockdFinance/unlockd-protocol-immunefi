...
contract UiPoolDataProvider is IUiPoolDataProvider {
    ...
    function getSimpleReservesData(
        ...
        AggregatedReserveData[]
            memory reservesData = new AggregatedReserveData[](reservesLength);
        ...
        returns (AggregatedReserveData[] memory, UserReserveData[] memory)
    {
        ...
        AggregatedReserveData[]
            memory reservesData = new AggregatedReserveData[](reservesLength);
        UserReserveData[] memory userReservesData = new UserReserveData[](
            user != address(0) ? reservesLength : 0
        );
    ...
    function _fillReserveData(
        ...
        reserveData.availableLiquidity = IERC20Detailed(
            reserveData.underlyingAsset
        ).balanceOf(reserveData.uTokenAddress);
    ...
    function _fillUserReserveData(
        ...
        userReserveData.uTokenBalance = IUToken(baseData.uTokenAddress)
            .balanceOf(user);
    ...
    function getSimpleNftsConfiguration(
        ...
        require(
            nftAssetsLength == nftTokenIds.length,
            Errors.LP_INCONSISTENT_PARAMS
        );

        ILendPool lendPool = ILendPool(provider.getLendPool());
        AggregatedNftConfiguration[]
            memory nftConfigs = new AggregatedNftConfiguration[](
                nftAssetsLength
            );
        ...
        for (uint256 i = 0; i < nftAssetsLength; ) {
            ...
            DataTypes.NftConfigurationMap memory baseConfig = lendPool
                .getNftConfigByTokenId(nftAssets[i], nftTokenIds[i]);

            _fillNftConfiguration(
                nftConfig,
                nftAssets[i],
                nftTokenIds[i],
                baseConfig
    ...
    function _fillNftConfiguration(
        ...
        nftConfig.priceInEth = nftOracle.getNFTPrice(
            nftConfig.underlyingAsset,
            nftConfig.tokenId
        );

        (
            nftConfig.ltv,
            nftConfig.liquidationThreshold,
            nftConfig.liquidationBonus
        ) = baseConfig.getCollateralParamsMemory();
        (
            nftConfig.redeemDuration,
            nftConfig.auctionDuration,
            nftConfig.redeemFine,
            nftConfig.redeemThreshold
        ) = baseConfig.getAuctionParamsMemory();
