...
contract RepayAndTransferHelper is ReentrancyGuard, Ownable {
    ...
    function repayETHAndTransferERC721(
        ...
        IWETHGateway wethGateway = IWETHGateway(
            addressProvider.getAddress(ADDRESS_ID_WETH_GATEWAY)
        );
        ...
        (uint256 repayAmount, ) = wethGateway.repayETH{value: totalDebt}(
            nftAsset,
            nftTokenId,
            totalDebt
        );
        ...
        require(
            IERC721(nftAsset).ownerOf(nftTokenId) == target,
            "owner is not target"
        );
    }
