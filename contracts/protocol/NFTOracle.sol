...
contract NFTOracle is INFTOracle, Initializable, OwnableUpgradeable {
   ...
   */
    function _addCollection(
        address _collection
    ) internal onlyNonExistingCollection(_collection) {
   ...
   */
    ...
    function setMultipleNFTPrices(
        ...
        uint256 collectionsLength = _collections.length;
        if (
            collectionsLength != _tokenIds.length ||
            collectionsLength != _prices.length
        ) revert ArraysLengthInconsistent();
        for (uint256 i = 0; i != collectionsLength; ) {
   ...
   */
    ...
    function getMultipleNFTPrices(
        ...
        uint256 collectionsLength = _collections.length;
        if (collectionsLength != _tokenIds.length)
            revert ArraysLengthInconsistent();
    ...
    function setPause(
        address _collection,
        bool paused
    ) external override onlyOwner onlyExistingCollection(_collection) {
        collectionPaused[_collection] = paused;
        emit CollectionPaused(paused);
    }

    function setPriceManagerStatus(
        address newPriceManager,
        bool val
    ) external onlyOwner {
        require(
            newPriceManager != address(0),
            Errors.NFTO_INVALID_PRICEM_ADDRESS
        );
