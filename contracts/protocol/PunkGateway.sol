    ...
    EmergencyTokenRecoveryUpgradeable
{
    ...
    function repay(
        uint256 punkIndex,
        uint256 amount
    ) external override nonReentrant returns (uint256, bool) {
    ...
    function liquidateNFTX(
        ...
        require(
            _addressProvider.getLendPoolLiquidator() == _msgSender(),
            Errors.CALLER_NOT_POOL_LIQUIDATOR
        );
        ...
        uint256 remainAmount = cachedPool.liquidateNFTX(
            address(wrappedPunks),
            punkIndex
        );
    ...
    function auctionETH(
        ...
        _wethGateway.auctionETH{value: msg.value}(
            address(wrappedPunks),
            punkIndex,
            onBehalfOf
        );
