 ...
 * @author Unlockd - Forked from BendDAO protocol (https://github.com/BendDAO/bend-lending-protocol)
 **/
    ...
    LendPoolStorage
{
    ...
    bytes32 public constant ADDRESS_ID_WETH_GATEWAY =
        0xADDE000000000000000000000000000000000000000000000000000000000001;
    bytes32 public constant ADDRESS_ID_PUNK_GATEWAY =
    ...
    modifier onlyHolder(address nftAsset, uint256 nftTokenId) {
        require(
            _msgSender() == IERC721Upgradeable(nftAsset).ownerOf(nftTokenId) ||
                _msgSender() ==
                IERC721Upgradeable(_nfts[nftAsset].uNftAddress).ownerOf(
                    nftTokenId
                ),
    ...
    modifier onlyCollection(address nftAsset) {
        ...
        require(
            data.uNftAddress != address(0),
            Errors.LP_COLLECTION_NOT_SUPPORTED
        );
        _;
    }
    ...
    function _onlyLendPoolLiquidatorOrGateway() internal view {
        require(
            _addressesProvider.getLendPoolLiquidator() == _msgSender() ||
                _addressesProvider.getAddress(ADDRESS_ID_WETH_GATEWAY) ==
                _msgSender() ||
                _addressesProvider.getAddress(ADDRESS_ID_PUNK_GATEWAY) ==
                _msgSender(),
    ...
    {
        require(
            _configFee == msg.value,
            Errors.MSG_VALUE_DIFFERENT_FROM_CONFIG_FEE
        );
    ...
    function getNftData(
        address asset
    ) external view override returns (DataTypes.NftData memory) {
        return _nfts[asset];
    }
    ...
    {
        DataTypes.NftConfigurationMap storage nftConfig = _nftConfig[nftAsset][
            nftTokenId
        ];

        DataTypes.ReserveData storage reserveData = _reserves[reserveAsset];

        (ltv, liquidationThreshold, liquidationBonus) = nftConfig
            .getCollateralParams();
        ...
        )
    {
        ...
        (uint256 ltv, uint256 liquidationThreshold, ) = nftConfig
            .getCollateralParams();
    ...
    function finalizeTransfer(
        ...
        require(
            _msgSender() == reserve.uTokenAddress,
            Errors.LP_CALLER_MUST_BE_AN_UTOKEN
        );
    ...
    function setLiquidateFeePercentage(
        uint256 percentage
    ) external override onlyLendPoolConfigurator {
     ...
     */
    function getLiquidateFeePercentage()
        external
        view
        override
        returns (uint256)
    ...
    function initReserve(
        ...
        require(
            uTokenAddress != address(0) &&
                debtTokenAddress != address(0) &&
                interestRateAddress != address(0),
    ...
    function setReserveInterestRateAddress(
        ...
        require(
            asset != address(0) && rateAddress != address(0),
            Errors.INVALID_ZERO_ADDRESS
        );
