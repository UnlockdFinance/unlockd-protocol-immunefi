...
makeSuite("LendPoolLoan: Modifiers", (testEnv: TestEnv) => {
  ...
  it("Tries to invoke initNft not being the Pool", async () => {
    ...
    await expect(loan.initNft(bayc.address, uBAYC.address)).to.be.revertedWith(
  ...
  it("Tries to invoke updateLoan not being the Pool", async () => {
    const { deployer, bayc, uBAYC, uWETH, loan } = testEnv;
    await expect(
      loan.updateLoan(deployer.address, "1", "1", "0", "1")
    ).to.be.revertedWith(CT_CALLER_MUST_BE_LEND_POOL);
  });
  ...
  it("Tries to invoke repayLoan not being the Pool", async () => {
    const { deployer, bayc, uBAYC, uWETH, loan } = testEnv;
    await expect(
      loan.repayLoan(deployer.address, "1", uBAYC.address, "1", "1")
    ).to.be.revertedWith(CT_CALLER_MUST_BE_LEND_POOL);
  });
  ...
  it("Tries to invoke auctionLoan not being the Pool", async () => {
    const { deployer, bayc, uBAYC, uWETH, loan } = testEnv;
    await expect(
      loan.auctionLoan(deployer.address, "1", deployer.address, "1", "0", "0")
    ).to.be.revertedWith(CT_CALLER_MUST_BE_LEND_POOL);
  });
  ...
  it("Tries to invoke redeemLoan not being the Pool", async () => {
    const { deployer, bayc, uBAYC, uWETH, loan } = testEnv;
    await expect(
      loan.redeemLoan(deployer.address, "1", "1", "1")
    ).to.be.revertedWith(CT_CALLER_MUST_BE_LEND_POOL);
  });
  ...
  it("Tries to invoke liquidateLoan not being the Pool", async () => {
    const { deployer, bayc, uBAYC, uWETH, loan } = testEnv;
    await expect(
      loan.liquidateLoan(deployer.address, "1", uBAYC.address, "1", "1")
    ).to.be.revertedWith(CT_CALLER_MUST_BE_LEND_POOL);
  });
  ...
  it("Tries to invoke liquidateLoanNFTX not being the Pool", async () => {
    const { deployer, bayc, uBAYC, uWETH, loan } = testEnv;
    await expect(
      loan.liquidateLoanNFTX("1", uBAYC.address, "1", "0")
    ).to.be.revertedWith(CT_CALLER_MUST_BE_LEND_POOL);
  });
});
