...
makeSuite("ReserveOracle", (testEnv: TestEnv) => {
  ...
  before(async () => {
    ...
    mockChainlinkOracle = await new MockChainlinkOracleFactory(
  ...
  it("ReserveOracle: Add Aggregator", async () => {
    ...
    await mockReserveOracle.addAggregator(
      addresses[0].address,
      mockChainlinkOracle.address
    );
    await expect(
      mockReserveOracle.addAggregator(ZERO_ADDRESS, mockChainlinkOracle.address)
    ).to.be.revertedWith("ReserveOracle: empty address");
    expect(await mockReserveOracle.priceFeedKeys(0)).eq(addresses[0].address);
    expect(await mockReserveOracle.getAggregator(addresses[0].address)).eq(
      mockChainlinkOracle.address
    );
    expect(await mockReserveOracle.getAggregator(addresses[1].address)).eq(
      ZERO_ADDRESS
    );
    await mockReserveOracle.addAggregator(
      addresses[1].address,
    ...
    await expect(
      mockReserveOracle.addAggregator(addresses[2].address, ZERO_ADDRESS)
    ).to.be.revertedWith("ReserveOracle: empty address");
  });
  ...
  it("ReserveOracle: get latest price", async () => {
    ...
    await expect(
      mockReserveOracle.getAssetPrice(addresses[1].address)
    ).to.be.revertedWith("key not existed");
    expect(
      await mockReserveOracle.getAssetPrice(addresses[0].address),
      "12345678"
    );
  ...
  it("ReserveOracle: get eth price", async () => {
    expect(
      await mockReserveOracle.getAssetPrice(testEnv.weth.address),
      "1000000000000000000"
    );
  });
  ...
  it("ReserveOracle: get latest timestamp", async () => {
    ...
    await mockChainlinkOracle.mockAddAnswer(8, 12345678, 1, 200000000000, 1);
    await expect(
      mockReserveOracle.getLatestTimestamp(addresses[1].address)
    ).to.be.revertedWith("ReserveOracle: empty address");
    expect(
      await mockReserveOracle.getLatestTimestamp(addresses[0].address),
      "200000000000"
    );
    await mockChainlinkOracle.mockAddAnswer(9, 100, 2, 200000000001, 2);
    expect(
  ...
  makeSuite("ReserveOracle-TWAP", () => {
    before(async () => {
      ...
      await mockChainlinkOracle.mockAddAnswer(
        100,
        4000000000000000,
        2,
        currentTime.add(15),
    ...
    it("twap price", async () => {
      const time = await mockReserveOracle.getLatestTimestamp(
        addresses[0].address
      );
      // (15*4100000000000000+15*4050000000000000+15*4000000000000000)/45 = 405
      const price = await mockReserveOracle.getTwapPrice(
        addresses[0].address,
        30
      );
    ...
    it("get 0 while interval is zero", async () => {
      await expect(
        mockReserveOracle.getTwapPrice(addresses[0].address, 0)
      ).to.be.revertedWith("interval can't be 0");
    });
  });
});
