...
task(
  "dev:deploy-all-mock-aggregators",
  "Deploy all mock aggregators for dev enviroment"
)
  ...
  .setAction(async ({ verify, pool }, localBRE) => {
    ...
    const allAggregatorsAddresses =
      await deployAllReservesMockAggregatorsInPoolConfig(poolConfig, verify);

    console.log("allAggregatorsAddresses:", allAggregatorsAddresses);
  });

task(
  "dev:deploy-mock-aggregator",
  "Deploy one mock aggregator for dev enviroment"
)
  .addFlag("verify", "Verify contracts at Etherscan")
  .addParam(
    "pool",
  ...
  .setAction(async ({ verify, pool, name, decimals, price }, localBRE) => {
    ...
    const mockAgg = await deployChainlinkMockAggregator(
      name,
      decimals,
      price,
      verify
