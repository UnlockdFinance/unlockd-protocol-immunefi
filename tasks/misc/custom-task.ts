...
task("dev:deposit-eth", "Doing custom task")
  ...
  .setAction(async ({ pool, amount }, DRE) => {
    ...
    console.log(
      "uWETH Balance:",
      (await uWETH.balanceOf(signerWithAddress.address)).toString()
    );
  });
...
task("dev:withdraw-eth", "Doing custom task")
  ...
  .setAction(async ({ pool, amount }, DRE) => {
    ...
    if (amount == "-1") {
      ...
      amountDecimals = await convertToCurrencyDecimals(
        signerWithAddress,
        weth,
        amount
      );
    ...
    const wethResData = await lendPool.getReserveData(weth.address);
    const uWETH = await getUToken(wethResData.uTokenAddress);
    const allowance = await uWETH.allowance(
      signerWithAddress.address,
      wethGateway.address
    );
    if (allowance.lt(amountDecimals)) {
      await waitForTx(
        await uWETH.approve(wethGateway.address, MAX_UINT_AMOUNT)
      );
    }
    ...
    await waitForTx(
      await wethGateway.withdrawETH(amountDecimals, signerWithAddress.address)
    );
  });
...
task("dev:borrow-eth-using-punk", "Doing custom task")
  ...
  .setAction(async ({ pool, id, amount }, DRE) => {
    ...
    let amountDecimals: BigNumberish;
    if (amount == "-1") {
      const unlockdDataProvider = await getUnlockdProtocolDataProvider(
        await addressesProvider.getUnlockdDataProvider()
      );
      const wethResData = await unlockdDataProvider.getReserveData(
        weth.address
      );
      console.log(
    ...
    if (notFalsyOrZeroAddress(punkAddress)) {
      if (punkAddress == signerWithAddress.address) {
        ...
        console.log(
          `Punk address ${punkAddress} is not owner ${signerWithAddress.address}`
        );
        borrowMore = true;
      }
    } else {
...
task("dev:repay-eth-using-punk", "Doing repay task")
  ...
  .setAction(async ({ pool, id, amount }, DRE) => {
    ...
    if (amount == "-1") {
      ...
      const loanData = await unlockdDataProvider.getLoanDataByCollateral(
        wpunk.address,
        id
      );
...
task("dev:debt-approve-delegate", "Doing custom task")
  ...
  .setAction(async ({ pool, asset }, DRE) => {
    ...
    const reserveToken = await unlockdDataProvider.getReserveTokenData(asset);
    const debToken = await getDebtToken(reserveToken.debtTokenAddress);

    console.log(
      "borrowAllowance before:",
      await debToken.borrowAllowance(
        await signer.getAddress(),
        wethGateway.address
      )
    );
    await waitForTx(
      await debToken.approveDelegation(wethGateway.address, MAX_UINT_AMOUNT)
    );
