...
task("dev:deploy-new-implementation", "Deploy new implementation")
  ...
  .setAction(async ({ verify, pool, contract, upgrade }, DRE) => {
    ...
    if (contract == "UnlockdProtocolDataProvider") {
      const contractImpl = await deployUnlockdProtocolDataProvider(
        addressesProvider.address,
        verify
      );
      console.log(
        "UnlockdProtocolDataProvider implementation address:",
        contractImpl.address
      );

      if (upgrade) {
        await waitForTx(
          await addressesProvider.setUnlockdDataProvider(contractImpl.address)
        );
      }
    }
    ...
    if (contract == "UnlockdCollector") {
      ...
      console.log(
        "UnlockdCollector implementation address:",
        contractImpl.address
      );
    }

    if (contract == "RepayAndTransferHelper") {
      const contractImpl = await deployRepayAndTransferHelper(
        addressesProvider.address,
        verify
      );
      console.log(
        "RepayAndTransferHelper implementation address:",
        contractImpl.address
      );
    }
  });
  ...
  .setAction(async ({ pool, contract, proxy, impl }, DRE) => {
    ...
    await waitForTx(
      await proxyAdmin
        ...
        .upgrade(unlockdProxy.address, impl)
    );
