...
task("dev:transfer-owner", "Transfer ownership")
  ...
  .setAction(async ({ verify, pool, contract, target }, DRE) => {
    ...
    const contractInst = OwnableFactory.connect(
      contract,
    ...
    const currentOwnerSigner = await getEthersSignerByAddress(
      currentOwnerAddress
    );
    await waitForTx(
      await contractInst.connect(currentOwnerSigner).transferOwnership(target)
    );
  });
